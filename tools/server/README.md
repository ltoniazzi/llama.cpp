...
| `--chat-truncate`| when the chat generation might exceed the context size, truncate by keeping the system message (and at least one user message) but dropping oldest turns until prompt fits within F*ctx_size tokens (0 < F < 1, default: disabled) |
